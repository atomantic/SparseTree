...
1. **Reverse god-file regression** — `PersonDetail.tsx` (1360), `ProviderDataTable.tsx` (1243), `database.service.ts` (1618), `auditor-agent.service.ts` (1227 — new checks now live in `auditor-checks.service.ts` instead, keeping this under its 1280 budget; the 7 original structural checks are still in the main file), `multi-platform-comparison.service.ts` (1099), `api.ts` (1249), `VerticalFamilyView.tsx` (977), `favorites.service.ts` (872), `person.routes.ts` (1119). Extract `usePersonData` / `usePersonOverrides` hooks, `PhotoThumbnail` / `ComparisonCell` / `ProviderRow` sub-components, and split `database.service.ts` along entity lines. Finish splitting `auditor-agent.service.ts`'s remaining structural checks (`checkImpossibleDates`, `checkParentAgeConflict`, `checkPlaceholderName`, `checkMissingGender`, `checkOrphanedEdges`, `checkUnlinkedProviders`, `checkDateMismatches`) into `auditor-checks.service.ts` alongside the new ones, leaving the main file as the walker + persistence + resolution engine.
2. **Critical-path unit tests** — `credentials.service.ts` (encryption), `validation.ts` (input sanitization), `errorHandler.ts`, `requestTimeout.ts`, `augmentation.service.ts` all currently have **zero** tests. `auditor-agent.service.ts` now has coverage for `missing_parents`/`stale_record`/`duplicate_suspect` (`tests/unit/services/auditorAgent.spec.ts`) but its 7 original structural checks (`impossible_date`, `parent_age_conflict`, `placeholder_name`, `missing_gender`, `orphaned_edge`, `unlinked_provider`, `date_mismatch`) are still untested. (`database.service.ts` and `search.service.ts` now have integration coverage but no unit tests of internal helpers.)
3. **Phase 18 remaining checks: `place_mismatch` / `name_mismatch`** — `missing_parents`, `duplicate_suspect`, and `stale_record` are done (see DONE.md). The two mismatch checks need `multiPlatformComparisonService.compareAcrossPlatforms(dbId, personId)`, which is `async` (reads provider cache from disk, and `getProviderData` — not used by `compareAcrossPlatforms` itself — can trigger a live browser scrape elsewhere in that service). `auditPerson()` and the BFS loop in `runAudit()` are currently synchronous, and `auditPath()` wraps its loop in `sqliteService.transaction(() => ...)`, which requires a **synchronous** callback (better-sqlite3) — so adding these two checks means either (a) making `auditPerson`/`runAudit`/`auditPath` async and restructuring `auditPath`'s transaction to insert issues in a separate synchronous pass after an async comparison pass, or (b) precomputing comparisons in a batch before the BFS walk. Filter `compareAcrossPlatforms()`'s `FieldComparison[]` result for `fieldName in ('name','birthPlace','deathPlace')` where `status === 'different'` — no new comparison logic needed, just the async plumbing.
...
- [ ] Implement `place_mismatch` / `name_mismatch` — see "Next Up" #3 for why they're blocked on making the audit loop async.
...
- [ ] `duplicate_suspect` currently buckets on exact normalized display name + birth year within 2 years — misses nickname variants (Jon/Jonathan) and off-by-a-few-years birth estimates. If it proves too narrow in practice, consider reusing `namesMatch()` from `multi-platform-comparison.service.ts` for the bucket key (pairwise across the whole tree, so watch the cost on large trees).
